    ...
    public function staffaction_por(Request $request)
    {
        ...
        try {
            ...
            if (!empty($emails)) {
                ...
                $sth = $pdo->prepare("SET NOCOUNT ON; EXEC mgr.xrl_save_mail_log ?, ?, ?, ?, ?, ?, ?, ?, ?, ?;");
                $sth->bindParam(1, $entity_cd);
                $sth->bindParam(2, $doc_no);
                $sth->bindParam(3, $request_type);
                $sth->bindParam(4, $type);
                $sth->bindParam(5, $module);
                $sth->bindParam(6, $sentTo);
                $sth->bindParam(7, $ccList);
                $sth->bindParam(8, $email_status);
                $sth->bindParam(9, $formattedDateTime);
                $sth->bindParam(10, 'MGR');
                $sth->execute();
                // var_dump($sth);
